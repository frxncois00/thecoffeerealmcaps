-- Keep customer payment-proof uploads aligned with the browser upload flow.
-- The client stages the object first, then attaches it to the order through the
-- three-argument RPC. Failed attachment attempts may remove only unreferenced
-- objects belonging to the signed-in customer.
