...
export function CheckoutPage(){
  ...
  const setPayment=value=>setForm(current=>({...current,payment:value,paymentReference:value===current.payment?current.paymentReference:''}));
  ...
  return <main className="customer-main checkout-page"><section className="page-title"><span>Secure checkout</span><h1>How should we prepare your order?</h1></section><div className="checkout-layout"><form className="checkout-form" onSubmit={submit}>
    ...
    <CheckoutSection n="3" title="Payment"><Choice title="Payment method" options={(systemSettings.payments.enabledMethods||[]).filter(method=>form.fulfillment==='delivery'||method!=='cod').map(method=>({id:method,name:method==='cod'?'Cash on delivery':method==='bank_transfer'?'Bank':'GCash'}))} value={form.payment} onChange={setPayment}/><CheckoutPaymentDetails payment={form.payment} paymentConfig={systemSettings.payments} total={total} referenceNumber={form.paymentReference} onReferenceChange={value=>set('paymentReference',value)}/></CheckoutSection>
...
function CheckoutPaymentDetails({payment,paymentConfig,total,referenceNumber,onReferenceChange}){
  ...
  return <section className="checkout-payment-details" aria-live="polite" aria-labelledby="checkout-payment-details-title">
    ...
    <div className="checkout-payment-details-body">
      ...
      <div className="checkout-payment-instructions">
        ...
        <ol><li>Send the exact amount shown above.</li><li>Save the transaction reference and a clear payment screenshot.</li><li>Upload the proof after reviewing your order.</li></ol>
      </div>
    </div>
...
function PaymentConfirmationModal({total,paymentConfig=SYSTEM_DEFAULTS.payments,busy,onClose,onConfirm}){
...
function ProofUploadModal({payment,busy,error,onBack,onSubmit,referenceNumber=''}){
  ...
  return <div className="payment-modal-backdrop"><section className="payment-modal proof-modal" role="dialog" aria-modal="true" aria-labelledby="proof-modal-title">
    ...
    <p>Upload a clear image showing the successful payment.</p>
    <div className="proof-reference-summary"><span>Reference number from checkout</span><b>{referenceNumber}</b></div>
    ...
    <div className="payment-modal-actions"><button className="secondary-button" type="button" onClick={onBack} disabled={busy}>Back to review</button><button className="primary-button" type="button" onClick={()=>file&&onSubmit(file,referenceNumber)} disabled={!file||busy}>{busy?'Uploading and placing order…':'Submit proof and order'}</button></div>
  </section></div>
}
...
export function OrderReviewPage(){
  ...
  const openPlacementFlow=()=>form.payment==='cod'?setModal('payment'):setModal('proof')
  ...
  return <main className="customer-main review-order-page">
    ...
    <button className="primary-button review-place-order" disabled={busy} onClick={()=>{setError('');openPlacementFlow()}}>{busy?'Preparing order…':'Place order'} <ArrowRight/></button>
    <p className="review-order-assurance">By placing your order, you confirm that the information above is correct.</p>
    {modal==='payment'&&<PaymentConfirmationModal paymentConfig={paymentConfig} total={total} busy={busy} onClose={()=>setModal(null)} onConfirm={()=>place()}/>}
    {modal==='proof'&&<ProofUploadModal payment={form.payment} busy={busy} error={error} referenceNumber={form.paymentReference} onBack={()=>{setError('');setModal(null)}} onSubmit={place}/>}
