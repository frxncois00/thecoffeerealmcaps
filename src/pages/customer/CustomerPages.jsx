...
function CheckoutPaymentDetails({payment,paymentConfig,total,referenceNumber,onReferenceChange,proof,previewUrl,proofError,onProofChange}){
  ...
  return <section className="checkout-payment-details" aria-live="polite" aria-labelledby="checkout-payment-details-title">
    ...
    <div className="checkout-proof-field"><span>Proof of payment</span><label className={`proof-dropzone${previewUrl?' has-preview':''}`}><input type="file" accept={IMAGE_UPLOAD_ACCEPT} required={!proof} onChange={onProofChange}/>{previewUrl?<><img className="proof-preview-image" src={previewUrl} alt="Selected payment proof preview"/><span className="proof-preview-action"><Camera size={17}/>Choose a different screenshot</span></>:<><ShoppingBag/><strong>Choose payment screenshot</strong><small>JPG, PNG, or WEBP only · Maximum 5 MB</small></>}</label>{proof&&<div className="proof-file" aria-live="polite"><span>{proof.name}</span><b>{(proof.size/1024/1024).toFixed(2)} MB</b></div>}{proofError&&<p className="field-hint error" role="alert">{proofError}</p>}</div>
  </section>
}
...
export function OrderReviewPage(){
  ...
  return <main className="customer-main review-order-page">
    ...
    <button className="primary-button review-place-order" disabled={busy} onClick={()=>{setError('');placeReviewedOrder()}}>{busy?'Uploading proof and placing order…':'Place order'} <ArrowRight/></button>
