  ...
  return <div className="ops-drawer-backdrop cancel-drawer-backdrop" onMouseDown={onClose}>
    <aside className="ops-drawer txn-drawer cancel-drawer" role="dialog" aria-modal="true" aria-label={`${view === 'refunds' ? 'Refund' : 'Cancellation'} record ${record.orderNumber}`} onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="ops-drawer-body">
        ...
        <section><h3>Amount breakdown</h3><div className="cancel-amount-list">{breakdown.isVatExemptDiscount?<>{breakdown.regularBaseAmount>0&&<p><span>VATable Sale</span><b>{money(breakdown.regularBaseAmount)}</b></p>}<p><span>VAT-Exempt Sale</span><b>{money(breakdown.vatExemptSale)}</b></p><p><span>{formatVatRate(pricing.vatRate)} VAT</span><b>{money(breakdown.regularVatAmount)}</b></p><p><span>Less 20% SC/PWD Disc.</span><b>- {money(breakdown.discountAmount)}</b></p></>:<><p><span>VATable Sale</span><b>{money(breakdown.baseAmount)}</b></p><p><span>{formatVatRate(pricing.vatRate)} VAT</span><b>{money(breakdown.vatAmount)}</b></p>{record.discountAmount>0&&<p><span>Discount</span><b>- {money(record.discountAmount)}</b></p>}</>}<p><span>Delivery fee</span><b>{money(record.deliveryFee)}</b></p><p className="total"><span>Original total</span><b>{money(record.originalAmount)}</b></p><p className="refund"><span>Refunded</span><b>{money(record.refundAmount)}</b></p></div></section>
