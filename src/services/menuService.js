...
const addonMatchesMenuItem=(addon,row)=>{
 if(Array.isArray(addon.subcategoryIds))return addon.subcategoryIds.includes(row.subcategory_id)
 const applies=addon.appliesTo||'both'
 return applies==='both'||applies===row.item_type
}
...
export async function fetchMenuCatalog(){
 ...
 const addons=[...(addonRows||[]).map(normalizeAddon),...foodAddonRows.map(row=>({id:row.id,name:row.name,price:Number(row.price),appliesTo:'food',targetTemperature:'both',subcategoryIds:null}))]
