...
export async function fetchCustomerOrders(userId){
  const {data,error}=await supabase.from('orders').select(ORDER_DETAIL_SELECT+',tracking_url').eq('customer_id',userId).order('created_at',{ascending:false})
  if(error)throw error
  return data||[]
}
export async function fetchCustomerOrder(orderId){
  const {data,error}=await supabase.from('orders').select(ORDER_DETAIL_SELECT+',tracking_url').eq('id',orderId).maybeSingle()
  if(error)throw error
  return data||null
}
