...
export async function exportTransactionsToXlsx({ transactions, summary, reconciliation, filterLabel, generatedBy }) {
  ...
  workbook.creator = generatedBy || 'The Coffee Realm'
  ...
  title.value = 'THE COFFEE REALM — TRANSACTION REPORT'
  ...
  summarySheet.getCell('B4').value = generatedBy || 'The Coffee Realm'
  ...
  ledgerTitle.value = 'THE COFFEE REALM — TRANSACTION LEDGER'
  ...
  ledger.getCell('A3').value = `Generated ${new Date().toLocaleString('en-PH')} by ${generatedBy || 'The Coffee Realm'}`
...
export async function exportTransactionsToPdf({ transactions, summary, reconciliation, filterLabel, generatedBy }) {
  ...
  const addHeader = (pageLabel) => {
    ...
    pdf.setTextColor(255, 255, 255); pdf.setFont('helvetica', 'bold'); pdf.setFontSize(15); pdf.text('THE COFFEE REALM — TRANSACTION REPORT', 38, 34)
    pdf.setTextColor(30, 57, 50); pdf.setFontSize(8); pdf.text(pageLabel, 38, 76)
  }
  addHeader(`Report period: ${filterLabel}`)
  pdf.setTextColor(70, 85, 76); pdf.setFont('helvetica', 'normal'); pdf.setFontSize(8); pdf.text(`Generated ${new Date().toLocaleString('en-PH')} by ${generatedBy || 'The Coffee Realm'}`, 38, 91)
