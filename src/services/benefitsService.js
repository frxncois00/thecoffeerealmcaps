import { customerSupabase, portalSupabase } from '../lib/supabase'
...
export async function fetchBenefitApplication(customerId) {
  const { data, error } = await customerSupabase.from('benefit_applications').select('*').eq('customer_id', customerId).maybeSingle()
  if (error) throw error
  return data
}
export async function fetchBenefitApplications(status, page = 1) {
  let query = portalSupabase.from('benefit_applications').select('*', { count: 'exact' }).order('submitted_at', { ascending: false }).order('id')
  if (status !== 'all') query = query.eq('status', status)
  const { data, count, error } = await query.range((page - 1) * 10, page * 10 - 1)
  if (error) throw error
  return { items: data || [], count: count || 0 }
}
export async function benefitDocumentUrl(path, scope = 'portal') {
  const client = scope === 'customer' ? customerSupabase : portalSupabase
  const { data, error } = await client.storage.from('benefit-documents').createSignedUrl(path, 300)
  if (error) throw error
...
export async function submitBenefitApplication(values, file, existingPath, consent) {
  ...
  if (file) {
    ...
    const { error } = await customerSupabase.storage.from('benefit-documents').upload(documentPath, file, { contentType: mimeType, upsert: false })
    if (error) throw error
  }
  if (!documentPath) throw new Error('Upload a clear photo of your valid ID.')
  const { data, error } = await customerSupabase.rpc('submit_benefit_application', {
